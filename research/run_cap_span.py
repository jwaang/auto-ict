"""Run one entry-timeframe cell. Timeframe from EXP_TF, output from EXP_OUT.

Cost drag is 1.02 points over the stop distance, so it falls as stops widen. A
coarser entry timeframe widens them structurally rather than arbitrarily, which
is what experiment 27 could not do.
...
OVERRIDES = {
    ...
    "tp_min_r": 0.5,
    "min_rr_ratio": 1.0,
    "bias_vote_rule": "majority",
    "max_concurrent_positions": int(os.environ["EXP_CAP"]),
}
...
with params.overrides(OVERRIDES):
    result = run_backtest(
        ...
        ticker="ES",
        entry_tf="15min",
        trade_start=pd.Timestamp(os.environ.get("EXP_START","2021-07-25"), tz="UTC"),
        trade_end=pd.Timestamp(os.environ.get("EXP_END","2024-12-31"), tz="UTC"),
        progress_every=10**9,
    )
